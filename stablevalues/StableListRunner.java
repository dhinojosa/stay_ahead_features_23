...
class Application {
    final Supplier<Logger> logger = StableValue.supplier(() -> LoggerFactory.getLogger(Application.class));
    private final int POOL_SIZE = 10;

    private final List<OrderController> ORDERS
        = StableValue.list(POOL_SIZE, x -> {
        logger.get().info("Retrieving Controller");
        logger.get().info("x: {}", x);
